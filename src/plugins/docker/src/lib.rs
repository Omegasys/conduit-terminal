...
/// Docker integration plugin for Conduit.
...
impl DockerPlugin {
    ...
    fn handle_custom_event(
        ...
        match name {
            ...
            "docker.container.started" => Ok(()),
            "docker.container.stopped" => Ok(()),
            "docker.container.removed" => Ok(()),
            "docker.image.updated" => Ok(()),
            _ => Ok(()),
        }
    }
}
...
pub extern "C" fn conduit_plugin_create() -> *mut dyn Plugin {
    Box::into_raw(Box::new(DockerPlugin::new()))
}
